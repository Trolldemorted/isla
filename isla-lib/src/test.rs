...
fn step<'ir, 'task, B: BV>(
    ...
    for successor in &successors {
        ...
        //print_register(&successor.frame, &shared_state.symtab, "zR14");
        //print_registers(&successor.frame, &shared_state.symtab);

        if read_register(&successor.frame, &shared_state.symtab, "z_PC") == 0x0000000000213BC0 {
            print_register(&successor.frame, &shared_state.symtab, "zR29");
        }
        // 0x0000000000212528
    }
    successors
